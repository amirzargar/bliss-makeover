...
const emptyForm = { name:'', phone:'', email:'', date_of_birth:'', notes:'' }
...
export default function Customers() {
  ...
  async function saveEdit() {
    if (!editForm.name.trim())  return alert('Enter customer name')
    if (!editForm.phone.trim()) return alert('Enter phone number')
    setSavingEdit(true)
  ...
  async function save() {
    ...
    else { setShowForm(false); setForm(emptyForm); fetchCustomers() }
    setSaving(false)
  }
  ...
  return (
    <div>
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-4 gap-2 mb-4">
            {['basic','silver','gold','platinum'].map(t => {
              ...
              return (
                <div key={t}
                  className={`${tier.bg} rounded-xl p-3 text-center cursor-pointer`}
              ...
              {filtered.map(c => {
                ...
                return (
                  <div key={c.id}
                    ...
                    <div className="w-9 h-9 rounded-full bg-pink-100 flex items-center justify-center text-pink-700 font-semibold text-sm flex-shrink-0">
        ...
        {/* Desktop side panel */}
        {selected && (
          <div className="hidden md:block w-80 flex-shrink-0 bg-white rounded-2xl border border-gray-100 p-5 overflow-y-auto self-start sticky top-0">
            <ProfileContent
              selected={selected}
              history={history}
              ...
              onClose={() => { setSelected(null); setShowEditForm(false) }}
      ...
      {selected && (
        <div className="md:hidden">
          <div className="fixed inset-0 bg-black/40 z-40" onClick={() => { setSelected(null); setShowEditForm(false) }} />
          <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl p-5 max-h-[85vh] overflow-y-auto">
...
function ProfileContent({
  ...
  showEditForm, editForm, setEditForm, savingEdit,
  onClose, onEdit, onSaveEdit, onCancelEdit, onDelete
}) {
  const tier = TIERS[selected.loyalty_tier] || TIERS.basic
    ...
    <>
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-pink-100 flex items-center justify-center text-pink-700 font-bold text-lg">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {!showEditForm && (
          <button onClick={onEdit}
            className="text-xs text-blue-500 hover:text-blue-700 font-medium border border-blue-200 px-3 py-1 rounded-lg hover:bg-blue-50">
            Edit Profile
          </button>
        )}
      </div>
      ...
      {showEditForm ? (
        <div className="space-y-3 mb-4">
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-3">
            <p className="text-xs text-blue-700 font-medium">Editing profile for {selected.name}</p>
          </div>
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Full name *</label>
        ...
        <>
          ...
          <div className="space-y-2 text-sm mb-4">
            {selected.email && (
              <div className="flex justify-between">
                ...
                <span className="text-gray-700 truncate ml-2 text-xs">{selected.email}</span>
              </div>
            )}
            {selected.date_of_birth && (
              <div className="flex justify-between">
                <span className="text-gray-400">Birthday</span>
                <span className="text-gray-700 text-xs">{selected.date_of_birth}</span>
              </div>
            )}
            {selected.notes && (
              <div>
                <span className="text-gray-400 block mb-1 text-xs">Notes</span>
                <p className="text-gray-600 text-xs bg-gray-50 rounded-lg p-2">{selected.notes}</p>
              </div>
            )}
            {!selected.email && !selected.date_of_birth && !selected.notes && (
              <p className="text-xs text-gray-300 text-center py-2">
                No additional details. Click Edit Profile to add.
              </p>
            )}
          </div>
          ...
          <button onClick={() => onDelete(selected.id)}
            className="w-full border border-red-200 text-red-400 py-1.5 rounded-lg text-xs hover:bg-red-50 transition-colors">
            Delete customer
          </button>
        </>
      )}
